...
- 조직 `admin`, `owner`: 팀 생성·이름 변경·삭제, 팀 멤버 관리
- Team `manager`: 자신이 관리하는 팀의 이름 변경, 기존 조직 멤버 배정과 team role 변경, 팀 멤버 제거
- Team `member`: team scope resource 읽기·쓰기

`설정`에서 조직 이름, 신규 회원 정책(즉시 활성화 또는 승인 대기), 기본 팀을 관리한다. 기본 팀이 설정되면 신규 회원이 활성화될 때 자동으로 해당 팀에 배정된다. 조직 삭제는 `owner`만 가능하며 조직의 모든 데이터를 함께 제거한다.
