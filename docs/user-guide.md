...
| 자동 처리 내역 확인·애매한 지식 검토 | AI 후보 검토 |
...
Knowledge extraction이 활성화되면 ready document chunk에서 entity와 relationship 후보가 생성된다. 추출과 별도의 AI 검증을 통과한 지식은 자동으로 Graph에 반영한다. 애매하거나 충돌하는 지식과 현재 strict 사전에 막힌 자동 승인 항목은 사람이 검토한다. 근거 없는·사소한 항목과 구체적인 사실을 나타내지 않는 관계는 자동 제외한다. 인용의 존재와 AI 판단은 사실 정확성을 보장하지 않으므로 중요한 판단에는 원문을 확인하라.

기본 `확인 필요한 지식` 탭은 AI가 판단을 보류했거나 현재 strict 사전에 막힌 동일 개체·관계를 한 항목으로 모아 이유와 문서별 근거를 비교한다. 자동 승인·제외 항목 수와 아직 검증하지 않은 추출 수도 표시한다. `AI 자동 검토 실행`을 누르면 현재 접근 가능한 기존 추출을 worker 대기열에 등록한다. 신규 추출은 별도 클릭 없이 검증한다. `처리 내역` 탭에서 최근 검증 50건의 판단 이유·원문·자동 또는 수동 처리 기록을 확인한다. 문서 이름이나 지식 이름으로 검색하고 페이지를 이동할 수 있다. 빈 추출 결과와 자동 처리가 끝난 항목, AI 검증 대기 항목은 수동 검토 목록에 표시하지 않는다. `청크별 검토`에서는 아직 검토 중인 전체 추출을 확인할 수 있다. 구체적인 관계와 인용 근거가 있는 항목을 먼저 표시하지만 이 순위는 정확성의 보증이 아니다.
...
스위치·선택형·숫자 입력으로 값을 수정한다. 각 항목은 직접 설정·배포 환경·기본값·미설정 중 현재 값의 출처를 표시한다. `기본 설정 사용`은 override 제거를 예약하고, 저장 전에는 취소할 수 있다. 비밀값은 기존 값을 표시하지 않는다. 교체할 때만 새 값을 입력하고 명시적인 `값 지우기`로 빈 값을 저장한다. API 주소를 바꿀 때는 연결된 API 키도 명시적으로 교체·제거해야 한다.

변경이 있으면 고정 저장 바에 변경 수와 재시작 대상 수가 표시된다. `변경 취소`는 마지막 저장 상태로 돌아간다. 저장 중에는 입력을 잠그고 실패해도 변경을 유지한다. 내부 링크로 다른 페이지에 이동하거나 새로고침·닫기를 할 때 미저장 변경을 확인한다. 분류 간 이동은 변경을 버리지 않는다. 재시작이 필요한 설정을 저장한 뒤에는 모든 application·worker 인스턴스를 재시작해야 하며 화면이 서버를 재시작하지는 않는다. `ALLOWED_EMAIL_DOMAINS`를 비우면 모든 email domain을 허용한다.
