...
기본 `통합 지식` 탭은 같은 범위의 동일 개체·관계를 한 항목으로 모아 문서별 근거를 비교한다. 문서 이름이나 지식 이름으로 검색하고 페이지를 이동할 수 있다. 빈 추출 결과와 검토가 끝난 항목은 표시하지 않는다. 구체적인 관계와 인용 근거가 있는 항목을 먼저 표시하지만 이 순위는 정확성의 보증이 아니다.
