...
`/settings?section=…`의 분류 메뉴는 조직 프로필·지식 사전과 설치 운영을 나눈다. 설치 운영은 전역 admin에게만 표시하며 조직 정책은 조직 관리자만 편집한다. 설치 설정은 용도별 카드와 전체 검색을 제공하고, domain의 모든 설정 항목을 한 번씩 노출한다. Secret은 빈 교체 입력과 명시적 지우기로 다루며 저장 요청에는 변경한 값과 복원 대상만 포함한다.

두 저장 범위는 분류 이동 중 편집 상태를 유지한다. 변경 수·적용 시점을 표시하는 저장 바, 변경 취소, 실패·재시도, 기본 설정 복원 예약과 취소를 제공한다. 설정 페이지의 한·영 문구는 동일한 입력·권한 계약을 사용한다.
