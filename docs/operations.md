...
1. `v*` tag push가 `.github/workflows/release.yml`을 시작한다. 서비스 컨테이너를 사용하는 `pnpm verify`, PostgreSQL integration, 인증 E2E는 GitHub-hosted Ubuntu runner에서 실행하고, 이후 release·image build·GitOps job은 self-hosted runner에서 실행한다.
