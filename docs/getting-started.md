...
설정 후 수집되는 문서의 chunk에서 후보를 만든다. 기존 ready 문서를 자동으로 탐색해 후보를 채우지는 않는다. 추출 후 별도 AI 검증과 원문 인용·권한 검사를 통과한 항목은 자동 반영하고, 불확실한 항목만 운영 콘솔의 `AI 후보 검토`에 남긴다. 기존 미검증·자동 처리 미완료 추출은 `AI 자동 검토 실행`으로 대기열에 등록할 수 있다. 저장된 추출·평가는 재사용하며 재추출하지 않는다. 문서가 `ready`여도 Graph 처리는 계속 진행될 수 있으므로 Graph 화면의 진척과 처리 내역을 확인하라.
