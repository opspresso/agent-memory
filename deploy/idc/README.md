...
기본 위치는 `/opt/compose/backup/agent-memory`이며 7일이 지난 backup directory를 제거한다. `.env.secrets`와 `.env.infrastructure`는 별도 secret manager에서 복구해야 한다. Database와 object storage 중 하나만 복원하면 document metadata와 원본이 불일치한다.
