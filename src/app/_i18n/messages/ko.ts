import type { Messages } from "./en";

export const ko: Messages = {
  "locale.label": "언어",
  "locale.change": "언어 변경",
  "theme.current": "테마: {name}",
  "theme.system": "시스템",
  "theme.light": "라이트",
  "theme.dark": "다크",
  "meta.description":
    "AI Agent의 Memory, RAG, Knowledge Graph를 연결하는 Context 플랫폼",
  "nav.guide": "가이드",
  "nav.group.workspace": "워크스페이스",
  "nav.search": "통합 검색",
  "nav.documents": "문서 수집",
  "nav.review": "AI 후보 검토",
  "nav.connect": "Agent 연결",
  "nav.group.administration": "관리",
  "nav.members": "회원",
  "nav.teams": "팀",
  "nav.settings": "설정",
  "shell.toggleNavigation": "내비게이션 열기/닫기",
  "shell.userMenu": "계정 메뉴",

  "home.badge": "공유 Context",
  "home.eyebrow": "사람과 Agent가 함께 쓰는 조직 지식",
  "home.title": "에이전트가 기억하고,",
  "home.titleSecond": "더 나은 답을 만듭니다.",
  "home.lede":
    "한 기업의 장기 Memory, 문서 근거, Knowledge Graph를 연결하는 설치형 Context 플랫폼입니다. 독립적으로 사용하거나 Agent Studio에 연결하고, 승인된 멤버와 Agent가 권한에 맞는 지식을 공유합니다.",
  "home.capability.memory": "Long-term Memory",
  "home.capability.memoryBody":
    "MCP로 기억을 저장·회상·잊고, 변경 이력과 출처·유효기간을 보존합니다.",
  "home.capability.rag": "Hybrid RAG",
  "home.capability.ragBody":
    "문서를 수집해 본문을 검색합니다. Embedding 모델을 설정하면 의미 검색을 함께 사용할 수 있습니다.",
  "home.capability.graph": "Knowledge Graph",
  "home.capability.graphBody": "AI 검증 기준을 통과한 지식은 자동 반영하고 애매한 항목만 검토합니다. 관계를 따라 원문 근거를 확인합니다.",
  "home.capability.sharing": "Flexible Sharing",
  "home.capability.sharingBody":
    "운영자가 멤버를 승인하고 조직·팀·개인 범위별로 읽기와 관리 권한을 적용합니다.",
  "login.eyebrow": "MEMORY WORKSPACE",
  "login.title": "Agent Memory 시작하기",
  "login.lede": "필요한 Memory와 Context를 한곳에서 만나보세요.",
  "login.enterprise": "Enterprise SSO로 로그인",
  "login.google": "Google로 로그인",
  "login.or": "또는",
  "login.signIn": "로그인",
  "login.signUp": "가입",
  "login.name": "이름",
  "login.email": "이메일",
  "login.password": "비밀번호",
  "login.createAccount": "계정 만들기",
  "login.emailSignIn": "이메일로 로그인",
  "login.requestFailed": "로그인 요청에 실패했습니다.",
  "login.providerUrlMissing": "인증 제공자 URL을 받지 못했습니다.",
  "login.failed": "로그인에 실패했습니다.",
  "login.notConfigured":
    "로그인 수단이 설정되지 않았습니다. 운영자에게 로그인 설정을 요청하세요.",
  "guide.metaDescription": "Agent Memory의 검색, 문서 수집, 지식 자동 검토, Knowledge Graph와 MCP 사용 가이드",
  "guide.metaTitle": "사용 가이드 · Agent Memory",
  "guide.console": "Console",
  "guide.eyebrow": "Product guide · 7 chapters",
  "guide.title": "기억을 넣는 법보다,",
  "guide.titleSecond": "다시 믿고 쓰는 법.",
  "guide.lede":
    "한 설치에서 한 기업의 지식을 관리합니다. 가입 요청과 운영자 승인부터 Memory·문서·Graph 활용, Agent 연결까지 안내합니다.",
  "guide.flowLabel": "Agent Memory Context 흐름",
  "guide.flow.store": "근거와 scope를 함께 저장",
  "guide.flow.discover": "필요한 Context를 정확하게 발견",
  "guide.flow.use": "출처를 아는 답변과 행동",
  "guide.contentsLabel": "Guide 목차",
  "guide.onThisPage": "이 페이지의 내용",
  "guide.chapter.start": "시작하기",
  "guide.chapter.search": "Context 검색",
  "guide.chapter.memory": "Memory 관리",
  "guide.chapter.documents": "문서 수집",
  "guide.chapter.graph": "Graph 탐색",
  "guide.chapter.review": "AI 후보 검토",
  "guide.chapter.connect": "Agent 연결",
  "login.approvalNotice": "신규 가입은 가입 요청으로 처리됩니다. 운영자 승인 후 조직 지식을 사용할 수 있습니다.",
  "guide.start.request": "가입 요청",
  "guide.start.requestNote": "설정된 로그인 수단으로 계정을 만듭니다.",
  "guide.start.approve": "운영자 승인",
  "guide.start.approveNote": "운영자가 회원 화면에서 요청을 검토하고 승인합니다.",
  "guide.start.use": "지식 활용",
  "guide.start.useNote": "승인 후 권한에 맞는 Memory·문서·Graph를 사용합니다.",
  "guide.start.operatorNote": "승인 대기 화면에서 새로고침으로 상태를 확인하세요. 최초 운영자는 설치 관리자가 설정하며, 차단·제거된 계정은 운영자의 접근 복구가 필요합니다.",
  "guide.scope.title": "저장할 때 기본 공유 범위를 선택합니다.",
  "onboarding.deniedTitle": "접근이 제한되었습니다",
  "onboarding.deniedBody": "현재 계정은 이 설치의 지식을 사용할 수 없습니다. 운영자에게 접근 권한 확인을 요청하세요.",
  "guide.start.title":
    "가입 요청과 승인부터 시작합니다.",
  "guide.start.body":
    "가입은 이 설치의 멤버가 되기 위한 요청입니다. 운영자가 승인하기 전에는 조직의 지식을 읽거나 저장할 수 없습니다. 별도로 조직을 만들거나 선택할 필요는 없습니다.",
  "guide.scope.organizationAudience": "조직 멤버",
  "guide.scope.teamAudience": "팀 멤버와 조직 관리자",
  "guide.scope.userAudience": "본인",
  "guide.scope.teamManager": "팀 manager 또는 조직 admin·owner",
  "guide.scope.userManager": "본인",
  "guide.scope.read": "읽기 · {audience}",
  "guide.scope.manage": "관리 · {manager}",
  "guide.start.note":
    "조직 전체에 쓰기·관리하려면 admin 또는 owner 권한이 필요합니다. 팀 범위는 팀 멤버와 조직 관리자가 접근하며, 개인 범위는 기본적으로 본인에게만 허용됩니다. Memory에 명시적인 ACL을 부여하면 지정한 사용자·팀도 허용된 작업을 수행할 수 있습니다.",
  "guide.search.title": "질문에 맞는 검색 면을 고릅니다.",
  "guide.search.body":
    "모든 지식에서 Memory, 문서 조각과 Knowledge node를 함께 검색하거나 결과 종류를 좁힙니다. 결과를 선택하면 목록을 유지한 채 전체 내용과 출처를 확인할 수 있습니다.",
  "guide.search.input": "검색어 입력",
  "guide.search.inputNote": "정책, 장애 대응, 시스템 관계",
  "guide.search.relevance": "결과 읽기",
  "guide.search.relevanceNote": "전체 내용, 공유 범위와 출처 확인",
  "guide.search.evidence": "근거 확인",
  "guide.search.evidenceNote": "Memory 또는 문서 조각의 원문 열기",
  "guide.search.note":
    "검색 진단은 필요할 때 펼쳐 봅니다. 상대 관련도는 같은 검색의 결과 간 비교이며 신뢰 확률이 아닙니다. 보관됨·만료·미래 유효 Memory와 미처리 문서는 검색에서 제외됩니다.",
  "guide.search.reranker":
    "reranker를 설정하면 모든 지식 검색과 MCP recall에 적용합니다. Memory·Documents·Graph 개별 검색은 키워드와 선택형 의미 검색 점수를 사용합니다. 재정렬이 실패하면 hybrid 순위로 결과를 제공합니다.",
  "guide.memory.title": "결정의 현재 상태와 변경 이유를 함께 남깁니다.",
  "guide.memory.body":
    "왼쪽 Memory 메뉴에서 새 Memory를 선택해 결정, 규칙이나 유용한 사실을 공유 범위와 함께 기록합니다. 목록에서 항목을 선택하면 내용과 출처를 읽을 수 있습니다. 권한이 있는 사용자는 수정과 Version 이력을 확인할 수 있습니다.",
  "guide.memory.initial": "최초 기록",
  "guide.memory.current": "현재 상태",
  "guide.memory.note":
    "변경 사유는 선택 사항이며 수정 전 version은 보존됩니다. 다른 사용자가 먼저 저장했다면 최신 version을 다시 불러온 뒤 저장합니다. 보관은 새 version으로 기록되고 일반 검색에서 제외됩니다.",
  "guide.documents.title": "원본은 보존하고, 검색은 chunk로 수행합니다.",
  "guide.documents.body": "문서 수집에서 UTF-8 text, Markdown, CSV, JSON 또는 XML을 업로드합니다. 공유 범위를 선택하고 처리 상태와 원문을 확인합니다. 사용 가능(ready)은 문서 검색 준비가 끝났다는 뜻이며, AI 지식 추출·검증은 별도로 진행됩니다.",
  "guide.documents.note":
    "처리 대기·처리 중 문서는 화면이 보이는 동안 최대 2분간 자동 갱신합니다. 중지 후에는 새로고침합니다. 실패한 문서는 오류를 확인한 뒤 쓰기 권한으로 처리 재시도를 선택합니다. 문서 보관에는 관리 권한이 필요합니다.",
  "guide.graph.title": "관계를 따라가되, 근거에서 멀어지지 않습니다.",
  "guide.graph.body":
    "왼쪽 Knowledge Graph 메뉴나 검색 결과의 관계 보기에서 탐색을 시작합니다. Node를 검색한 뒤 지도 또는 node 목록에서 선택하고, 연결 관계와 원문 근거를 확인합니다. 선택한 node를 새로운 탐색 중심으로 바꿀 수 있습니다.",
  "guide.graph.note": "노드를 드래그해 위치를 고정하고 배경을 드래그해 이동합니다. 휠·핀치로 확대하며 화면 맞춤·배치 초기화·전체 화면을 사용할 수 있습니다. 전체 화면은 Esc로 종료합니다. 키보드 노드 목록과 읽을 수 있는 원문 근거도 함께 제공합니다.",
  "guide.review.title": "기준을 통과한 지식은 자동 반영하고, 애매한 항목만 확인합니다.",
  "guide.review.body": "지식 추출 모델과 문서 worker가 설정되면 추출과 별도의 AI 검증으로 원문 인용, 유용성, 충돌과 사전 기준을 확인합니다. 같은 공유 범위의 반복된 개체·관계는 출처를 모아 보여줍니다. AI 판단과 인용 확인이 사실 정확성을 보장하지는 않습니다.",
  "guide.review.accept": "자동 승인",
  "guide.review.acceptNote": "명시적이고 유용하며 원문 근거와 정책 기준을 통과한 항목을 Graph에 반영하고 출처를 누적합니다.",
  "guide.review.reject": "자동 제외",
  "guide.review.rejectNote": "근거가 없거나 사소한 항목, 구체적인 사실을 나타내지 않는 관계는 Graph에 반영하지 않습니다.",
  "guide.graph.progress": "Graph의 처리 진척은 추출·검증·자동 처리 상태를 보여주며 수동 검토 완료를 뜻하지 않습니다. 검색어 우선 처리는 이미 추출된 이름·별칭에 맞는 미완료 후보를 대상으로 합니다. 관계 새로고침으로 새 연결을 확인하세요.",
  "guide.review.manual": "확인 필요한 지식",
  "guide.review.manualNote": "불확실성·충돌·사전 제약이 있는 항목은 이유와 원문을 비교해 반영할 출처를 선택합니다.",
  "guide.review.partial": "선택한 출처의 지식만 승인·거절할 수 있습니다. 관계 승인은 필요한 양 끝 개체를 함께 반영하고, 개체 거절은 아직 검토하지 않은 연결 관계도 거절합니다. 다른 항목은 대기 상태로 남고 이미 승인한 Graph는 거절로 삭제되지 않습니다.",
  "guide.review.history": "AI 자동 검토 실행은 기존 미검증·자동 처리 미완료 후보를 대기열에 등록합니다. 청크별 검토에서 남은 추출 묶음을, 처리 내역에서 최근 검증 50건과 자동·수동 판단 기록을 확인하세요. 빈 추출은 기본 검토 목록에 표시하지 않습니다.",
  "guide.connect.title": "MCP로 Agent와 Context를 연결합니다.",
  "guide.connect.body":
    "서비스는 /api/mcp로 장기 기억을 저장·회상·잊습니다. Agent Memory는 RAG 문서와 Knowledge Graph도 관리하며, context_search로 세 종류를 함께 검색합니다. Agent 연결에서 endpoint와 인증 정보를 확인하고, 필요하면 Agent Studio 등록 템플릿을 사용합니다.",
  "guide.connect.note":
    "조직 admin·owner는 Agent token을 관리합니다. 사용자 위임이 없으면 조직 범위만 허용하며, 신뢰된 서비스가 X-User-Email을 보내면 활성 멤버의 권한을 적용합니다. Studio의 자동 Memory 회상은 version에 서버를 연결하고 memoryRecall 설정과 recall 도구를 허용해야 동작합니다.",
  "guide.next.title": "이제 실제 Context를 탐색하세요.",
  "guide.next.body":
    "Memory를 만들거나 문서를 업로드해 시작하세요. 검색 결과에서 원문 근거를 확인하고 준비된 지식을 Agent에 연결합니다.",
  "guide.next.action": "Console 열기",
  "organization.requestFailed": "요청을 처리하지 못했습니다.",
  "organization.loadFailed": "조직 정보를 불러오지 못했습니다.",
  "organization.memberSaved": "조직 멤버를 저장했습니다.",
  "organization.teamCreated": "팀을 만들었습니다.",
  "organization.teamMemberSaved": "팀 멤버를 저장했습니다.",
  "organization.manageTitle": "조직 관리",
  "organization.manageBody": "가입된 계정을 조직과 팀에 배정합니다.",
  "organization.refresh": "새로고침",
  "organization.members": "조직 멤버",
  "organization.registeredEmail": "가입된 사용자 이메일",
  "organization.role": "조직 역할",
  "organization.saveMember": "멤버 저장",
  "organization.createTeam": "팀 만들기",
  "organization.teamName": "팀 이름",
  "organization.teamSlug": "팀 slug",
  "organization.teamMembers": "팀 멤버",
  "organization.team": "팀",
  "organization.memberEmail": "조직 멤버 이메일",
  "organization.teamRole": "팀 역할",
  "organization.saveTeamMember": "팀 멤버 저장",

  "organization.name": "조직 이름",
  "organization.slug": "조직 slug",


  "onboarding.title": "접근 승인 대기",
  "onboarding.lede":
    "가입이 요청으로 접수되었습니다. 운영자가 승인하면 멤버가 되어 Memory를 사용할 수 있습니다.",




  "onboarding.pendingNotice": "조직 관리자의 승인을 기다리고 있습니다.",

  "members.title": "회원 관리",
  "members.lede": "회원을 확인하고 역할·팀을 변경하거나 접근을 관리합니다.",
  "members.accessDenied": "조직 관리자만 볼 수 있는 페이지입니다.",
  "members.column.member": "회원",
  "members.column.role": "역할",
  "members.column.status": "상태",
  "members.column.teams": "팀",
  "members.column.actions": "작업",
  "members.roleOf": "{email}의 역할",
  "members.actionsFor": "{email}에 대한 작업",
  "members.status.active": "활성",
  "members.status.pending": "승인 대기",
  "members.status.blocked": "차단됨",
  "members.approve": "승인",
  "members.approved": "회원을 승인했습니다.",
  "members.block": "차단",
  "members.blocked": "회원을 차단했습니다.",
  "members.unblock": "차단 해제",
  "members.remove": "조직에서 제거",
  "members.removed": "회원을 제거했습니다.",
  "members.removeTitle": "회원 제거",
  "members.removeBody":
    "{email}을(를) 조직에서 제거할까요? 팀 소속과 접근 권한은 제거되지만 user scope 데이터는 관리자의 재등록을 위해 보존됩니다.",
  "members.assignTeam": "{email}을(를) 팀에 배정",
  "members.addTeam": "팀 추가",
  "members.removeFromTeam": "{team}에서 제거",
  "members.teamRemoved": "팀 배정을 해제했습니다.",
  "teams.title": "팀 관리",
  "teams.lede": "팀을 만들고 팀원과 역할을 관리합니다.",
  "teams.empty": "아직 팀이 없습니다.",
  "teams.noMembers": "이 팀에는 아직 팀원이 없습니다.",
  "teams.rename": "팀 이름 변경",
  "teams.renamed": "팀 이름을 변경했습니다.",
  "teams.renameTitle": "팀 이름 변경",
  "teams.delete": "팀 삭제",
  "teams.deleted": "팀을 삭제했습니다.",
  "teams.deleteTitle": "팀 삭제",
  "teams.deleteBody":
    "{name} 팀을 삭제할까요? 팀 소속과 team scope의 Database record가 영구히 삭제됩니다. 업로드 원본은 storage lifecycle에 따라 별도로 정리됩니다.",
  "settings.field.DOCUMENT_WORKER_ENABLED": "문서 자동 처리",
  "settings.field.BETTER_AUTH_URL": "서비스 주소",
  "settings.field.AUTH_PASSWORD": "이메일·비밀번호 로그인",
  "settings.field.AUTH_PASSWORD_SIGNUP": "비밀번호 계정 가입",
  "settings.field.ALLOWED_EMAIL_DOMAINS": "허용할 이메일 도메인",
  "settings.field.ADMIN_EMAILS": "설치 관리자 이메일",
  "settings.field.GOOGLE_CLIENT_ID": "Google 클라이언트 ID",
  "settings.field.GOOGLE_CLIENT_SECRET": "Google 클라이언트 비밀키",
  "settings.field.OIDC_ISSUER": "인증 제공자 주소",
  "settings.field.OIDC_CLIENT_ID": "OIDC 클라이언트 ID",
  "settings.field.OIDC_CLIENT_SECRET": "OIDC 클라이언트 비밀키",
  "settings.field.OIDC_SCOPES": "요청할 인증 범위",
  "settings.field.EMBEDDING_BASE_URL": "Embedding API 주소",
  "settings.field.EMBEDDING_API_KEY": "Embedding API 키",
  "settings.field.EMBEDDING_MODEL": "Embedding 모델",
  "settings.field.RERANKER_BASE_URL": "Reranker API 주소",
  "settings.field.RERANKER_API_KEY": "Reranker API 키",
  "settings.field.RERANKER_MODEL": "Reranker 모델",
  "settings.field.RERANKER_TIMEOUT_MS": "응답 대기 시간",
  "settings.field.RERANKER_MIN_SCORE": "최소 관련도",
  "settings.field.KNOWLEDGE_EXTRACTION_BASE_URL": "지식 추출 API 주소",
  "settings.field.KNOWLEDGE_EXTRACTION_API_KEY": "지식 추출 API 키",
  "settings.field.KNOWLEDGE_EXTRACTION_MODEL": "추출 모델",
  "settings.field.KNOWLEDGE_VERIFICATION_BASE_URL": "지식 검증 API 주소",
  "settings.field.KNOWLEDGE_VERIFICATION_MODEL": "검증 모델",
  "settings.field.KNOWLEDGE_VERIFICATION_API_KEY": "지식 검증 API 키",
  "settings.field.KNOWLEDGE_EXTRACTION_LANGUAGE": "지식 추출 언어",
  "settings.extractionLanguage.source": "원문 언어",
  "settings.extractionLanguage.ko": "한국어",
  "settings.extractionLanguage.en": "영어",
  "settings.field.KNOWLEDGE_ENRICHMENT_CONCURRENCY": "지식 추출 동시 처리 수",
  "settings.field.AI_PROVIDER_MAX_CONCURRENCY": "AI 동시 요청 수",
  "settings.field.AI_PROVIDER_REQUESTS_PER_MINUTE": "인스턴스별 분당 요청",
  "settings.field.AI_ORGANIZATION_REQUESTS_PER_MINUTE": "조직별 분당 요청",
  "settings.field.AI_USER_REQUESTS_PER_MINUTE": "사용자별 분당 요청",
  "settings.field.DOCUMENT_STORAGE_QUOTA_BYTES": "원본 문서 저장 한도",
  "settings.field.DOCUMENT_PENDING_QUOTA": "처리 대기·진행 문서 한도",
  "settings.field.DOCUMENT_UPLOADS_PER_USER_PER_HOUR": "사용자별 시간당 업로드",
  "settings.field.S3_ENDPOINT": "스토리지 주소",
  "settings.field.S3_REGION": "리전",
  "settings.field.S3_BUCKET": "문서 버킷",
  "settings.field.S3_ACCESS_KEY_ID": "액세스 키 ID",
  "settings.field.S3_SECRET_ACCESS_KEY": "시크릿 액세스 키",
  "settings.field.S3_FORCE_PATH_STYLE": "경로 기반 버킷 주소",
  "settings.field.LOG_LEVEL": "로그 상세 수준",
  "settings.field.METRICS_BEARER_TOKEN": "Metrics 접근 토큰",
  "settings.field.LANGFUSE_PUBLIC_KEY": "Langfuse 공개 키",
  "settings.field.LANGFUSE_SECRET_KEY": "Langfuse 비밀키",
  "settings.field.LANGFUSE_BASE_URL": "Langfuse 주소",
  "settings.field.LANGFUSE_EXPORT_MODE": "Trace 전송 방식",
  "settings.field.LANGFUSE_TRACING_ENVIRONMENT": "Trace 환경 이름",
  "settings.ux.nav.general": "조직 프로필",
  "settings.ux.nav.ontology": "지식 사전",
  "settings.ux.nav.ai": "AI 모델",
  "settings.ux.nav.document": "문서 처리",
  "settings.ux.nav.auth": "인증 및 접근",
  "settings.ux.nav.storage": "스토리지",
  "settings.ux.nav.observability": "로그 및 모니터링",
  "settings.ux.description.general": "조직의 이름과 신규 회원이 소속될 기본 팀을 관리합니다.",
  "settings.ux.description.ontology": "지식 그래프에 사용할 개체·관계 용어와 검증 기준을 정의합니다.",
  "settings.ux.description.ai": "의미 검색, 결과 재정렬, 지식 추출에 사용할 모델을 연결합니다.",
  "settings.ux.description.document": "문서 처리 방식과 저장·업로드 한도를 조정합니다.",
  "settings.ux.description.auth": "로그인 수단과 이 설치에 접근할 수 있는 사용자를 관리합니다.",
  "settings.ux.description.storage": "문서 원본을 보관할 S3 호환 스토리지를 연결합니다.",
  "settings.ux.description.observability": "로그, 메트릭과 AI 실행 추적을 구성합니다.",
  "settings.ux.recommendationsUnavailable": "추천 용어를 불러오지 못했습니다. 직접 편집과 저장은 계속할 수 있습니다.",
  "settings.ux.searchResults": "설치 설정 검색 결과",
  "settings.ux.searchResultsBody": "모든 설치 설정에서 일치하는 항목을 표시합니다. 변경한 설정은 분류와 관계없이 함께 저장합니다.",
  "settings.ux.title": "설정",
  "settings.ux.lede": "조직의 작업 방식부터 AI와 운영 환경까지, 한곳에서 관리하세요.",
  "settings.ux.admin": "설치 관리자",
  "settings.ux.organizationAdmin": "조직 관리",
  "settings.ux.navigation": "설정 분류",
  "settings.ux.organization": "조직 관리",
  "settings.ux.application": "설치 운영",
  "settings.ux.scopeNote": "조직 정책과 설치 전체의 설정은 별도로 저장합니다.",
  "settings.ux.retry": "다시 시도",
  "settings.ux.loading": "설정 불러오는 중",
  "settings.ux.search": "설치 설정 검색 · 이름 또는 환경 변수",
  "settings.ux.clearSearch": "검색 지우기",
  "settings.ux.noResults": "일치하는 설정이 없습니다.",
  "settings.ux.searchHint": "다른 이름이나 환경 변수명으로 검색해 보세요.",
  "settings.ux.source.override": "직접 설정",
  "settings.ux.source.env": "배포 환경",
  "settings.ux.source.default": "기본값",
  "settings.ux.source.unset": "미설정",
  "settings.ux.modified": "변경됨",
  "settings.ux.restoring": "복원 대기",
  "settings.ux.restore": "기본 설정 사용",
  "settings.ux.undoRestore": "복원 취소",
  "settings.ux.restoreHint": "저장하면 직접 설정한 값을 제거하고 배포 환경 또는 기본값을 사용합니다.",
  "settings.ux.sourceHint": "직접 설정한 값이 배포 환경·기본값보다 우선합니다.",
  "settings.ux.clear": "값 지우기",
  "settings.ux.clearValue": "{name} 값 지우기",
  "settings.ux.clearPending": "저장하면 기존 비밀값을 빈 값으로 바꿉니다.",
  "settings.ux.secretKeep": "기존 값 유지 · 교체할 때만 입력",
  "settings.ux.secretNew": "새 비밀값 입력",
  "settings.ux.enabled": "사용",
  "settings.ux.disabled": "사용 안 함",
  "settings.ux.automatic": "자동 · 실행 환경 기본값",
  "settings.ux.discard": "변경 취소",
  "settings.ux.unsaved": "저장하지 않은 변경 {count}개",
  "settings.ux.noChanges": "변경 사항 없음",
  "settings.ux.restartCount": "이 중 {count}개는 저장 후 서버 재시작이 필요합니다.",
  "settings.ux.restartTitle": "재시작 후 적용되는 변경이 있습니다.",
  "settings.ux.restartBody": "설정은 저장됐습니다. 재시작이 필요한 설정을 적용하려면 운영자가 모든 application·worker 인스턴스를 재시작해야 합니다. 이 화면은 서버를 재시작하지 않습니다.",
  "settings.ux.organizationSaveHint": "조직 프로필과 지식 사전의 변경을 함께 저장합니다.",
  "settings.ux.leave": "저장하지 않은 설정 변경이 있습니다. 변경을 버리고 이동하시겠습니까?",
  "settings.ux.group.embedding": "의미 검색",
  "settings.ux.groupBody.embedding": "모델을 설정하면 키워드와 의미 검색을 함께 사용합니다. API 주소를 바꿀 때는 키를 다시 입력하거나 지워야 합니다.",
  "settings.ux.group.reranker": "검색 결과 재정렬",
  "settings.ux.groupBody.reranker": "통합 검색과 Memory 회상 결과를 재정렬합니다. 주소와 모델을 함께 설정하며 최소 관련도를 비우면 순위만 조정합니다.",
  "settings.ux.group.extraction": "지식 추출과 자동 검토",
  "settings.ux.group.verification": "독립 검증 모델",
  "settings.ux.groupBody.verification": "추출과 다른 모델로 원문 근거와 지식 표현을 검증할 수 있습니다. 비워 두면 추출 모델을 사용합니다. 별도로 지정할 때는 주소와 모델을 함께 설정하고 해당 서비스의 API 키를 입력합니다.",
  "settings.ux.groupBody.extraction": "문서에서 지식을 추출하고 별도 검증을 실행합니다. 주소를 바꿀 때는 API 키도 다시 입력하거나 지웁니다.",
  "settings.ux.group.limits": "AI 사용 한도",
  "settings.ux.groupBody.limits": "요청 폭주를 방지하는 한도입니다. 조직·사용자 한도는 여러 인스턴스가 공유합니다.",
  "settings.ux.group.worker": "백그라운드 처리",
  "settings.ux.groupBody.worker": "문서 worker가 새 문서를 처리합니다. 지식 추출은 1–16개를 동시에 실행하며 AI 동시 요청 한도를 넘지 않습니다.",
  "settings.ux.group.quotas": "저장 및 업로드 한도",
  "settings.ux.groupBody.quotas": "원본 저장량과 처리 대기열을 제한합니다. 문서를 보관해도 원본 저장 용량은 줄지 않습니다.",
  "settings.ux.group.access": "서비스 접근 정책",
  "settings.ux.groupBody.access": "도메인은 쉼표로 구분합니다. 비우면 모든 이메일 도메인을 허용합니다. 관리자 목록에는 본인 이메일이 필요합니다.",
  "settings.ux.group.password": "이메일 로그인",
  "settings.ux.groupBody.password": "최소 한 가지 로그인 수단을 유지해야 합니다. 비밀번호 신규 가입은 로컬 개발 환경에서만 허용합니다.",
  "settings.ux.group.google": "Google 로그인",
  "settings.ux.groupBody.google": "클라이언트 ID와 비밀키를 함께 설정합니다.",
  "settings.ux.group.oidc": "기업 SSO",
  "settings.ux.groupBody.oidc": "인증 제공자 주소, 클라이언트 ID와 비밀키를 함께 설정합니다. 범위는 공백으로 구분합니다.",
  "settings.ux.group.bucket": "문서 저장 위치",
  "settings.ux.groupBody.bucket": "MinIO 등 S3 호환 저장소를 사용할 수 있습니다. 저장 위치를 바꿔도 기존 원본 파일이 자동 이동되지는 않습니다.",
  "settings.ux.group.storageCredentials": "스토리지 인증",
  "settings.ux.groupBody.storageCredentials": "액세스 키 ID와 시크릿 키를 함께 설정합니다. 기존 값은 노출하지 않습니다.",
  "settings.ux.group.logging": "로그와 메트릭",
  "settings.ux.groupBody.logging": "로그 수준과 Prometheus 접근 토큰을 관리합니다. Metrics 토큰은 32자 이상이며 비우면 endpoint를 비활성화합니다.",
  "settings.ux.group.tracing": "AI 실행 추적",
  "settings.ux.groupBody.tracing": "Langfuse 공개 키와 비밀키를 함께 설정합니다. 전송 방식이 미설정이면 실행 환경의 기본값을 사용합니다.",
  "settings.title": "조직 설정",
  "settings.lede": "조직 프로필과 기본 팀을 관리합니다.",
  "settings.general": "일반",
  "settings.save": "설정 저장",
  "settings.saved": "설정을 저장했습니다.",
  "settings.application.title": "애플리케이션 설정",
  "settings.application.body":
    "Database override가 환경 변수보다 우선합니다. 접근 설정은 즉시 적용되고 별도 표시된 항목은 재시작 후 적용됩니다.",
  "settings.application.section.auth": "인증 및 접근",
  "settings.application.section.ai": "AI provider 및 제한",
  "settings.application.section.document": "문서 처리",
  "settings.application.section.storage": "Object storage",
  "settings.application.section.observability": "로그 및 telemetry",
  "settings.application.restartRequired": "저장 후 재시작 필요",
  "settings.application.resetPending": "env 사용",
  "settings.application.useEnv": "{name}에 환경 변수 값 사용",
  "settings.application.save": "설치 설정 저장",
  "settings.application.saved": "애플리케이션 설정을 저장했습니다.",
  "settings.application.loadFailed": "애플리케이션 설정을 불러오지 못했습니다.",
  "settings.application.saveFailed": "애플리케이션 설정을 저장하지 못했습니다.",




  "settings.defaultTeam.label": "기본 팀",
  "settings.defaultTeam.description":
    "신규 회원이 활성화될 때 자동으로 배정되는 팀입니다.",
  "settings.defaultTeam.placeholder": "기본 팀 없음",







  "settings.ontology.title": "온톨로지",
  "settings.ontology.body":
    "Knowledge Graph에서 허용할 node kind와 edge predicate를 정의합니다.",
  "settings.ontology.mode.label": "검증 모드",
  "settings.ontology.mode.description":
    "사전에 없는 용어를 사용한 knowledge 쓰기를 처리하는 방식입니다.",
  "settings.ontology.mode.off": "사용 안 함 · 검증하지 않음",
  "settings.ontology.mode.warn": "경고 · 미등록 용어 표시",
  "settings.ontology.mode.strict": "엄격 · 미등록 용어 거부",
  "settings.ontology.nodeKinds.label": "Node kind 사전",
  "settings.ontology.nodeKinds.description":
    "허용하는 entity kind입니다. Enter로 추가하며 소문자로 저장됩니다.",
  "settings.ontology.nodeKinds.placeholder": "service, database, concept",
  "settings.ontology.edgePredicates.label": "Edge predicate 사전",
  "settings.ontology.edgePredicates.description":
    "허용하는 관계 predicate입니다. Enter로 추가하며 소문자로 저장됩니다.",
  "settings.ontology.edgePredicates.placeholder": "depends_on, owns, uses",
  "settings.ontology.termCount": "{count}/{max}개 용어",
  "settings.ontology.recommend.title": "문서 기반 추천",
  "settings.ontology.recommend.body":
    "이 조직의 추출된 knowledge에서 관찰된 용어입니다. 클릭하면 사전에 추가됩니다.",
  "settings.ontology.suggest.button": "AI 추천",
  "settings.ontology.suggest.body":
    "추출 모델에게 관찰된 용어의 정제·통합을 요청합니다.",
  "settings.ontology.suggest.title": "AI 추천 용어",
  "settings.ontology.suggest.empty": "모델이 새로 제안할 용어를 찾지 못했습니다.",
  "workspace.resultFallback": "검색 결과",
  "workspace.selectOrganization": "조직 접근 권한을 확인하세요",
  "workspace.searchFailed": "검색에 실패했습니다.",
  "workspace.graphFailed": "관계 지도를 불러오지 못했습니다.",
  "workspace.uploadFailed": "업로드에 실패했습니다.",
  "workspace.uploadQueueFailed":
    "문서는 저장했지만 처리 대기열 등록에 실패했습니다. API에서 retry하세요: {id}",
  "workspace.uploadQueued": "수집 대기열에 등록했습니다: {id}",
  "workspace.signOutFailed": "로그아웃에 실패했습니다.",
  "workspace.eyebrow": "운영 Console",
  "workspace.title": "공유 Context를 한곳에서 관리합니다.",
  "workspace.signOut": "로그아웃",
  "workspace.noOrganization": "접근 가능한 조직이 없습니다.",

  "workspace.requestMembership": "관리자에게 멤버십을 요청하세요.",
  "workspace.activeOrganization": "활성 조직",
  "workspace.tab.search": "통합 검색",
  "workspace.tab.upload": "문서 수집",
  "workspace.tab.connect": "Agent 연결",
  "workspace.tab.review": "AI 후보 검토",
  "workspace.tab.manage": "조직 관리",
  "workspace.searchPlaceholder": "정책, 장애 대응, 시스템 관계를 검색하세요",
  "workspace.search": "검색",
  "workspace.relativeRelevance": "상대 관련도 {value}%",
  "workspace.viewRelationships": "관계 보기",
  "workspace.nodeSources": "source {count}개",
  "workspace.searchEmpty":
    "검색어를 입력하면 권한 범위 안의 Context가 표시됩니다.",
  "workspace.mapEyebrow": "Knowledge map",
  "workspace.mapTitle": "연결된 지식을 탐색합니다.",
  "workspace.mapCount": "{nodes} nodes · {edges} edges",
  "workspace.uploadTitle": "문서 수집",
  "workspace.uploadFormats": "UTF-8 text, Markdown, JSON, XML, CSV · 최대 10 MiB",
  "workspace.scope.user": "개인 · 나만 사용",
  "workspace.scope.team": "팀 · 선택한 팀과 공유",
  "workspace.scope.organization": "조직 · 모든 조직 멤버와 공유",
  "workspace.scopeLabel": "공유 범위",
  "workspace.shareTeam": "공유할 팀",
  "workspace.selectTeam": "팀을 선택하세요",
  "workspace.documentTitle": "문서 제목",
  "workspace.documentTitlePlaceholder": "파일명을 기본값으로 사용",
  "workspace.documentFile": "문서 파일",
  "workspace.startIngestion": "수집 시작",
  "workspace.mcpBody":
    "이 조직 endpoint를 Streamable HTTP MCP server로 등록합니다.",
  "workspace.studioTemplateTitle": "Agent Studio 등록 템플릿",
  "workspace.studioTemplateBody":
    "Agent Studio의 Tools → Register MCP server에서 아래 값을 해당 항목에 붙여 넣으세요. Name은 version에서 연결할 서버의 식별자입니다. Description은 모델과 capability 검색에 사용되고, 선택 입력인 Content는 운영자에게만 표시됩니다.",
  "workspace.studioTemplateDescription":
    "{organizationSlug} 조직에 저장된 결정·규칙·경험, 문서 근거, 지식 간 관계를 확인할 때 사용합니다. Memory·RAG·Knowledge Graph를 검색하고 공유할 정보를 조직 범위 Memory로 저장합니다.",
  "workspace.studioTemplateContent":
    "# {organizationSlug} 조직 Agent Memory 운영 메모\n\n- 접근 범위: X-User-Email 없는 Agent token은 조직 범위만 허용한다. X-User-Email을 보내면 해당 활성 멤버의 권한을 적용한다. 이 header는 사용자 신원을 검증하는 신뢰된 서버만 설정하라. Studio는 로그인 사용자 email을 전달하며 project·대화 header는 접근 범위를 부여하지 않는다.\n- 제공 기능: Memory는 remember·recall·forget으로 저장·회상·잊는다. forget은 manage 권한과 현재 version을 검증해 archive한다. Memory·RAG·Graph 통합 검색은 context_search를 사용한다. 문서 수집과 Graph 관리는 Agent Memory 콘솔에서 수행한다.\n- 연결 확인: Test connection은 도구 목록 조회를 검사한다. 저장 후 사용할 project의 version에 서버를 직접 연결하고 실제 실행도 확인한다.\n- 자동 회상: version의 memoryRecall을 켜고 recall 도구를 허용한다. Memory만 검색하며 설정된 reranker를 적용한다. 자동 검색으로 추가된 서버는 실행 전 회상 대상이 아니다.\n- Token 변경: 재생성하면 Studio의 Authorization header를 갱신한다. Version에 같은 header를 덮어쓴 경우 해당 값도 갱신한다.\n\n이 Content는 모델에 전달되지 않는 운영자 메모다. 정보 저장 조건과 응답 규칙은 version의 system prompt 또는 연결한 Skill에 작성한다.",
  "workspace.studioTemplateCopyField": "등록 템플릿 {field} 복사",
  "workspace.studioTemplateTokenHint":
    "Headers의 Key는 Authorization, Value는 Bearer 뒤에 공백과 실제 Agent token을 입력합니다. <amt_token>을 위에서 생성하거나 확인한 token으로 교체하세요.",
  "workspace.studioTemplateNextSteps":
    "등록 후 Test connection으로 도구 목록을 확인하고 사용할 project의 version에 서버를 직접 연결하세요. 실행 전 Memory 회상이 필요하면 memoryRecall을 켜고 recall 도구를 허용하세요. 모델이 따라야 할 저장·응답 규칙은 version의 system prompt 또는 Skill에 작성하세요.",
  "workspace.agentTokenTitle": "Agent token",
  "workspace.agentTokenBody":
    "신뢰된 서비스에서 사용할 Bearer token을 생성합니다. X-User-Email이 없으면 조직 범위만 허용하고, 있으면 해당 활성 멤버의 권한을 적용합니다. 발급자는 활성 admin·owner 권한을 유지해야 합니다.",
  "workspace.agentTokenGenerate": "Token 생성",
  "workspace.agentTokenRegenerate": "Token 재생성",
  "workspace.agentTokenReveal": "Token 보기",
  "workspace.agentTokenHide": "Token 숨기기",
  "workspace.agentTokenRevoke": "Token 폐기",
  "workspace.agentTokenRevokeConfirm":
    "Agent token을 폐기할까요? 연결된 Agent가 즉시 동작하지 않게 됩니다.",
  "workspace.agentTokenCopyNow":
    "지금 token을 복사하거나 나중에 Token 보기로 다시 확인할 수 있습니다.",
  "workspace.agentTokenRegenerateToReveal":
    "이 token은 hash만 저장되어 있습니다. 보기를 활성화하려면 한 번 재생성하세요.",
  "workspace.agentTokenAdminOnly":
    "조직 admin 또는 owner만 Agent token을 관리할 수 있습니다.",
  "workspace.agentTokenFailed": "Agent token 요청에 실패했습니다.",
  "workspace.copyEndpoint": "MCP endpoint 복사",
  "workspace.copy": "복사",
  "workspace.copied": "복사됨",
  "resource.archiveDocument": "문서 Archive",
  "resource.archiveDocumentTitle": "문서 Archive",
  "resource.archiveDocumentBody":
    "{name}을 Archive할까요? 원본 근거는 보존되지만 검색, retry, AI 후보 검토에서 제외됩니다.",
  "resource.deleteGraphTitle": "Graph resource 삭제",
  "resource.deleteGraphBody":
    "{name}을 영구 삭제할까요? Node를 삭제하면 연결 edge도 삭제되며 원본 Memory와 문서는 유지됩니다.",
  "resource.documentArchived": "{name}을 Archive했습니다.",
  "resource.graphDeleted": "Graph에서 {name}을 삭제했습니다.",
  "resource.deleteFailed": "Resource를 제거하지 못했습니다.",
  "resource.cancel": "취소",
  "resource.confirmArchive": "Archive 확인",
  "resource.confirmDelete": "삭제 확인",
  "resource.mergeDuplicate": "중복 병합",
  "resource.mergeNodesTitle": "중복 node 병합",
  "resource.mergeNodesBody":
    "중복된 {name} node를 병합할까요? Source와 관계를 현재 카드의 node로 옮긴 뒤 다른 node를 삭제합니다.",
  "resource.mergeReason": "병합 사유",
  "resource.mergeReasonPlaceholder":
    "두 node가 같은 entity라고 판단한 근거를 기록하세요",
  "resource.confirmMerge": "병합 확인",
  "resource.nodesMerged": "중복된 {name} node를 병합했습니다.",
  "result.evidence.directMemory": "직접 등록된 Memory",
  "result.evidence.memorySource": "{source} 출처{revision}",
  "result.evidence.revision": " · revision v{version}",
  "result.evidence.memoryKnowledge": "Memory에서 연결된 지식",
  "result.evidence.documentKnowledge": "문서 근거에서 연결된 지식",
  "result.evidence.graph": "Knowledge Graph 근거",
  "graph.resetLayout": "배치 초기화",
  "graph.gestureHint": "노드 드래그로 위치 고정 · 배경 드래그로 이동 · 휠·핀치로 확대 · 우클릭으로 노드 메뉴 · Ctrl+클릭으로 다중 선택",
  "graph.fullscreen": "전체 화면",
  "graph.exitFullscreen": "전체 화면 종료",
  "graph.mapLabel": "Knowledge Graph 관계 지도",
  "graph.searchLabel": "Graph node 검색",
  "graph.searchPlaceholder": "Node 찾기",
  "graph.zoomOut": "축소",
  "graph.zoomIn": "확대",
  "graph.fit": "화면에 맞춤",
  "graph.summary": "{nodes}개 node와 {edges}개 관계",
  "graph.kindFilter": "Node 종류 필터",
  "graph.relations": "{count} relations",
  "graph.noSummary": "이 node에는 아직 요약이 없습니다.",
  "graph.aliases": "검증된 별칭",
  "graph.connectedBy": "연결 관계",
  "graph.noRelations": "직접 연결된 관계가 없습니다.",
  "graph.nodeDetails": "상세 보기",
  "graph.selectionSummary": "선택한 노드 {nodes}개 · 관계 {edges}개",
  "graph.unpinNode": "고정 해제",
  "graph.selectNodeHint": "노드를 선택하면 연결 관계와 원문 근거를 볼 수 있습니다.",
  "graph.expandFromNode": "확장",
  "graph.exploreFromNode": "탐색",
  "graph.deleteNode": "Node 삭제",
  "graph.deleteEdge": "관계 삭제",
  "candidate.requestFailed": "후보 검토 요청을 처리하지 못했습니다.",
  "candidate.loadFailed": "Knowledge 후보를 불러오지 못했습니다.",
  "candidate.accepted": "후보를 공유 Knowledge Graph에 반영했습니다.",
  "candidate.rejected": "후보를 거절했습니다.",
  "candidate.eyebrow": "Curation gate",
  "candidate.title": "AI가 찾은 지식을 검토합니다.",
  "candidate.lede":
    "원문에서 추출한 후보는 승인하기 전까지 공유 Graph에 나타나지 않습니다.",
  "knowledgeProgress.refresh": "관계 새로고침",
  "knowledgeProgress.title": "지식 그래프 구성이 아직 완료되지 않았습니다",
  "knowledgeProgress.body": "전체 {total}개 청크 중 추출 {extracted}개 · 검증 및 자동 반영 {completed}개 완료",
  "knowledgeProgress.partial": "지금 보이는 결과는 처리 완료된 일부 지식입니다.",
  "knowledgeProgress.prioritize": "이 검색어 관련 지식 우선 처리",
  "knowledgeProgress.retry": "미완료 지식 처리 재시도",
  "knowledgeProgress.retried": "미완료 청크 {count}개를 처리하도록 등록했습니다.",
  "knowledgeProgress.prioritized": "관련 추출 {count}개를 우선 처리하도록 등록했습니다.",
  "knowledgeProgress.unavailable": "지식 처리 진행 상태를 확인하지 못했습니다.",
  "reviewQueue.history": "처리 내역",
  "reviewQueue.historyDescription": "최근 검증한 추출 50개의 판단 이유와 자동·수동 처리 기록을 확인합니다.",
  "reviewQueue.noHistory": "아직 AI 검증 내역이 없습니다.",
  "reviewQueue.acceptedDecision": "승인",
  "reviewQueue.ignoredDecision": "제외",
  "reviewQueue.pendingDecision": "처리 대기",
  "reviewQueue.automatic": "자동",
  "reviewQueue.human": "사람",
  "reviewQueue.automate": "AI 자동 검토 실행",
  "reviewQueue.autoPolicy": "기준을 통과한 지식은 자동 반영하고 불확실한 항목만 검토합니다.",
  "reviewQueue.autoAccepted": "자동 승인 {count}개",
  "reviewQueue.autoIgnored": "자동 제외 {count}개",
  "reviewQueue.unassessed": "AI 검증 전 추출이 {count}개 있습니다. 자동 검토를 실행하면 백그라운드에서 처리합니다.",
  "reviewQueue.queued": "{count}개 추출을 자동 검토 대기열에 등록했습니다. 처리 후 새로고침하세요.",
  "reviewQueue.groupTab": "확인 필요한 지식",
  "reviewQueue.chunkTab": "청크별 검토",
  "reviewQueue.lede": "AI가 자동 처리하지 못한 지식을 모아 판단 이유와 근거를 비교합니다.",
  "reviewQueue.completed": "후보 {count}개의 선택 항목을 처리했습니다.",
  "reviewQueue.partial": "{count}개 후보의 처리는 완료됐으나 나머지는 처리하지 못했습니다. 목록을 갱신했습니다.",
  "reviewQueue.entity": "개체",
  "reviewQueue.relationship": "관계",
  "reviewQueue.support": "출처 {count}곳 · 문서 {documents}개",
  "reviewQueue.explanation": "선택한 출처의 이 지식만 처리합니다. 관계 승인 시 양 끝 개체도 승인하며 다른 지식은 검토 대기로 남습니다. 같은 이름이라도 사건·시점이 다른지 근거를 확인하세요.",
  "reviewQueue.weak": "관계가 구체적이지 않습니다. 단순히 함께 등장한 것인지 원문에서 확인하세요.",
  "reviewQueue.legacy": "문장별 인용 근거가 없는 추출입니다. 원문을 열어 확인하세요.",
  "reviewQueue.selectAll": "출처 전체 선택",
  "reviewQueue.aliases": "별칭: {names}",
  "reviewQueue.source": "원문 펼치기 / 접기",
  "reviewQueue.reject": "선택한 지식 거절",
  "reviewQueue.accept": "선택한 출처 {count}곳 승인",
  "reviewQueue.search": "지식·문서 이름 검색",
  "reviewQueue.searchButton": "찾기",
  "reviewQueue.total": "검토할 지식 {count}개 · 추출 기록 {sources}개",
  "candidate.refresh": "새로고침",
  "candidate.emptyTitle": "검토할 후보가 없습니다.",
  "candidate.emptyBody":
    "문서 수집과 AI 분석이 끝나면 source가 확인된 후보가 여기에 쌓입니다.",
  "candidate.noEntity": "추출된 entity 없음",
  "candidate.counts": "{entities} entities · {relations} relations",
  "candidate.proposal": "AI proposal",
  "candidate.noKnowledge": "지식 없음 후보",
  "candidate.documentChunk": "Document chunk",
  "candidate.noRelationships": "제안된 관계가 없습니다.",
  "candidate.similarNodes":
    "같은 scope와 이름의 기존 node가 {count}개 있습니다(kind: {kinds}). 정규화 identity가 정확히 같으면 자동 병합되며, 다르면 승인 후 검토하여 병합하세요.",
  "candidate.reason": "검토 사유",
  "candidate.reasonPlaceholder": "승인 또는 거절 판단의 근거를 기록하세요",
  "candidate.reject": "거절",
  "candidate.accept": "Graph에 승인",
  "candidate.ontologyUnknown": "온톨로지에 없음",
  "candidate.ontologyWarn":
    "온톨로지 사전에 없는 용어가 포함되어 있습니다: {terms}",
  "candidate.ontologyStrict":
    "온톨로지 사전에 없는 용어가 있어 승인할 수 없습니다: {terms}. 조직 설정에서 사전을 갱신하세요.",
  "memory.requestFailed": "Memory 요청을 처리하지 못했습니다.",
  "memory.etagMissing": "Memory version header가 없습니다.",
  "memory.loadFailed": "Memory를 불러오지 못했습니다.",
  "memory.conflict": "다른 사용자가 먼저 수정했습니다. 최신 version을 다시 불러오세요.",
  "memory.revisionSaved": "새 revision을 저장했습니다.",
  "memory.revisionFailed": "Revision을 저장하지 못했습니다.",
  "memory.archived": "Memory를 archive했습니다.",
  "memory.archiveFailed": "Memory를 archive하지 못했습니다.",
  "memory.lifecycle": "Memory lifecycle",
  "memory.stateFailed": "현재 상태를 반영하지 못했습니다.",
  "memory.reload": "다시 불러오기",
  "memory.source": "source {source}",
  "memory.currentSnapshot": "현재 snapshot",
  "memory.validFrom": "유효 시작",
  "memory.expires": "만료",
  "memory.noExpiration": "기한 없음",
  "memory.newRevision": "새 revision",
  "memory.revisionHint": "저장 시 현재 version을 기준으로 충돌을 확인합니다.",
  "memory.title": "제목",
  "memory.content": "내용",
  "memory.changeReason": "변경 사유",
  "memory.changeReasonPlaceholder":
    "다음 사용자가 변경 맥락을 이해할 수 있게 기록하세요",
  "memory.saveRevision": "Revision 저장",
  "memory.archiveWarning": "검색에서 Memory를 제외합니다.",
  "memory.archiveBody":
    "Archive는 새 version으로 기록되며 현재 검색 결과에서 사라집니다.",
  "memory.cancel": "취소",
  "memory.confirmArchive": "Archive 확인",
  "memory.versionSpine": "Version 이력",
  "memory.historyTitle": "변경 맥락을 시간순으로 추적합니다.",
  "memory.current": "현재",
  "memory.updated": "{date} 수정",
  "nav.memory": "Memory",
  "nav.graph": "Knowledge Graph",
  "design.skipToContent": "본문으로 바로가기",

  "evidence.loading": "검토 대기 목록 불러오는 중",
  "evidence.queue": "검토 대기 · {count}",
  "evidence.original": "원문 근거",
  "evidence.entities": "제안된 지식",
  "evidence.relationships": "제안된 관계",
  "evidence.nodeList": "노드 목록 · {count}",
  "evidence.noNodes": "일치하는 노드가 없습니다.",
  "evidence.sources": "원문 근거 · {count}",
  "source.title": "원문 근거",
  "source.unavailable": "원문을 사용할 수 없거나 접근 권한이 변경되었습니다.",
  "source.retry": "다시 시도",
  "source.loading": "원문 근거 불러오는 중",
  "source.open": "원문 열기",
  "source.chunk": "본문 {number}",
  "manageUi.addMember": "회원 추가",
  "manageUi.addMemberBody": "가입된 계정을 추가한 뒤 아래 목록에서 역할, 접근 상태와 팀을 관리합니다.",
  "manageUi.teamBody": "팀이 함께 사용할 공유 범위를 만듭니다. 아래에서 팀을 선택해 회원을 관리합니다.",
  "manageUi.connectBody": "조직 지식을 Agent에 연결합니다. Endpoint를 복사하고 인증 정보를 설정한 뒤 서버를 등록합니다.",
  "manageUi.operatorNotes": "선택 입력인 운영자 메모 보기",
  "manageUi.searchMembers": "이름 또는 이메일 검색",
  "manageUi.allStatuses": "모든 상태",
  "manageUi.memberCount": "회원 {count}명",
  "manageUi.noMembers": "조건에 맞는 회원이 없습니다.",

  "documentUi.eyebrow": "지식 수집",
  "documentUi.title": "문서 라이브러리",
  "documentUi.description": "원문을 수집하고 처리 상태를 확인하여 검색 가능한 근거로 만드세요.",
  "documentUi.library": "문서",
  "documentUi.count": "표시 중인 문서 {count}개",
  "documentUi.loadFailed": "문서를 불러오지 못했습니다. 새로고침하여 다시 시도하세요.",
  "documentUi.loading": "문서 불러오는 중",
  "documentUi.actionFailed": "문서 작업을 완료하지 못했습니다.",
  "documentUi.refresh": "새로고침",
  "documentUi.emptyTitle": "첫 원문 문서를 추가하세요",
  "documentUi.emptyBody": "문서를 업로드하면 내용을 검색하고 지식의 근거로 활용할 수 있습니다.",
  "documentUi.more": "더 보기",
  "documentUi.selectTitle": "문서를 선택하세요",
  "documentUi.selectBody": "처리 상태, 공유 범위와 원문 정보를 확인할 수 있습니다.",
  "documentUi.format": "형식",
  "documentUi.size": "크기",
  "documentUi.created": "생성일",
  "documentUi.updated": "변경일",
  "documentUi.attempts": "처리 시도",
  "documentUi.processingError": "문서 처리 실패",
  "documentUi.source": "출처",
  "documentUi.findContent": "문서 검색 열기",
  "documentUi.retry": "처리 재시도",
  "documentUi.retryQueued": "재처리를 요청했습니다. 작업이 시작되면 상태가 갱신됩니다.",
  "documentUi.archive": "문서 보관",
  "documentUi.scope.change": "공유 범위 변경",
  "documentUi.scope.description": "문서와 모든 출처의 공개 범위가 확인된 지식만 함께 변경합니다. 다른 비공개 출처가 있거나 지식이 충돌하면 해당 지식의 기존 범위를 유지합니다.",
  "documentUi.scope.apply": "범위 적용",
  "documentUi.scope.saved": "문서 공유 범위를 변경했습니다.",
  "documentUi.scope.result": "연결된 지식 변경 결과",
  "documentUi.scope.nodes": "개체: 변경 {updated}개 · 동일 범위 {unchanged}개 · 제외 {skipped}개",
  "documentUi.scope.edges": "관계: 변경 {updated}개 · 동일 범위 {unchanged}개 · 제외 {skipped}개",
  "documentUi.scope.node": "개체",
  "documentUi.scope.edge": "관계",
  "documentUi.scope.reason.access_denied": "지식 관리 권한 없음",
  "documentUi.scope.reason.source_scope": "다른 출처의 공개 범위 불충족",
  "documentUi.scope.reason.source_unavailable": "사용할 수 없는 출처 포함",
  "documentUi.scope.reason.identity_conflict": "대상 범위에 동일한 지식 존재",
  "documentUi.scope.reason.endpoint_scope": "관계 끝점의 공개 범위 또는 출처 불충족",
  "documentUi.scope.reason.connected_edge": "기존 연결 관계의 공개 범위 유지 필요",
  "documentUi.scope.reapply": "제외된 지식은 기존 범위를 유지합니다. 다른 출처의 공유나 충돌을 해결한 뒤 같은 범위를 다시 적용하면 재검증합니다.",
  "documentUi.scope.conflict": "문서가 변경되었습니다. 새로고침하여 최신 범위를 확인한 뒤 다시 적용하세요.",
  "documentUi.scope.relatedConflict": "연결된 지식이 더 넓은 범위에 남아 있어 문서를 제한할 수 없습니다. 관련 지식의 공유 충돌을 먼저 해결하세요. 문서와 지식은 변경되지 않았습니다.",
  "documentUi.scope.unavailable": "문서의 상태나 접근 권한이 변경되었습니다. 새로고침하여 확인하세요.",
  "documentUi.archiveConfirm": "문서를 보관할까요? 문서 내용과 이 문서에 의존하는 지식은 검색에서 제외됩니다.",
  "documentUi.archived": "문서를 보관했습니다.",
  "documentUi.cancel": "취소",
  "documentUi.uploaded": "문서를 업로드했습니다. 상세 화면에서 처리 상태를 확인하세요.",
  "documentUi.pollPaused": "자동 갱신을 2분 후 중지했습니다. 새로고침하여 최신 상태를 확인하세요.",
  "documentUi.status.pending": "처리 대기",
  "documentUi.status.processing": "처리 중",
  "documentUi.status.ready": "사용 가능",
  "documentUi.status.failed": "실패",
  "documentUi.status.archived": "보관됨",
  "documentUi.help.pending": "작업 시작을 기다리고 있습니다. 화면이 보이는 동안 상태를 자동 갱신합니다.",
  "documentUi.help.processing": "문서 내용을 읽고 검색 색인을 만들고 있습니다. 상태를 자동 갱신합니다.",
  "documentUi.help.ready": "검색하거나 지식의 원문 근거로 활용할 수 있는 문서입니다.",
  "documentUi.help.failed": "오류 원인을 확인한 뒤 처리를 다시 시도하세요.",
  "documentUi.help.archived": "검색에서 제외된 문서입니다.",

  "memoryUi.read": "내용과 출처",
  "memoryUi.edit": "수정",
  "memoryUi.description": "결정, 규칙, 유용한 지식을 기록하고 사람과 Agent가 함께 활용합니다.",
  "memoryUi.create": "새 Memory",
  "memoryUi.createHint": "다시 활용할 지식을 기록하고 공유할 범위를 선택하세요.",
  "memoryUi.createFailed": "Memory를 만들지 못했습니다. 입력한 내용은 유지됩니다.",
  "memoryUi.kind": "지식 유형",
  "memoryUi.kind.rule": "규칙",
  "memoryUi.kind.experience": "경험",
  "memoryUi.kind.decision": "결정",
  "memoryUi.kind.preference": "선호",
  "memoryUi.kind.fact": "사실",
  "memoryUi.privateHint": "본인만 이 Memory에 접근할 수 있습니다.",
  "memoryUi.sharedHint": "이 조직의 활성 멤버가 이 Memory를 읽을 수 있습니다.",
  "memoryUi.teamHint": "선택한 팀의 멤버와 조직 관리자가 이 Memory에 접근할 수 있습니다.",
  "memoryUi.teamLoadFailed": "공유할 팀 목록을 불러오지 못했습니다.",
  "memoryUi.loadFailed": "Memory를 불러오지 못했습니다. 다시 시도하세요.",
  "memoryUi.search": "Memory 검색",
  "memoryUi.results": "검색 결과 · {count}",
  "memoryUi.recent": "최근 Memory · {count}",
  "memoryUi.loading": "Memory 불러오는 중",
  "memoryUi.noResults": "검색 결과가 없습니다",
  "memoryUi.noResultsHint": "다른 검색어를 입력하거나 검색어를 지워 최근 Memory를 확인하세요.",
  "memoryUi.empty": "첫 Memory를 기록해 보세요",
  "memoryUi.emptyHint": "기억할 결정, 업무 규칙, 유용한 사실을 기록하세요.",
  "memoryUi.previous": "이전",
  "memoryUi.next": "다음",
  "memoryUi.back": "목록으로",
  "memoryUi.select": "Memory를 선택하세요",
  "memoryUi.selectHint": "전체 내용과 출처를 확인하고 권한에 따라 수정하거나 변경 이력을 살펴보세요.",
  "searchUi.description": "공유 지식을 찾고 원문 근거와 연결된 관계를 확인합니다.",
  "searchUi.ingest": "문서 추가",
  "searchUi.all": "모든 지식",
  "searchUi.openDocument": "문서 상세 열기",
  "searchUi.ranking": "검색 진단 정보",
  "searchUi.rankingHint": "상대 관련도는 이번 검색 결과 안에서만 비교하는 값이며 신뢰도나 정확도를 뜻하지 않습니다.",
  "searchUi.backResults": "검색 결과로 돌아가기",
  "searchUi.loading": "지식 검색 중",
  "searchUi.results": "검색 결과 {count}건",
  "searchUi.resultsLabel": "검색 결과 목록",
  "searchUi.detail": "지식 상세",
  "searchUi.closeDetail": "목록으로 돌아가기",
  "searchUi.selectTitle": "근거를 확인하세요",
  "searchUi.selectBody": "결과를 선택하면 전체 내용과 출처, 가능한 작업을 확인할 수 있습니다.",
  "searchUi.noResults": "일치하는 지식이 없습니다",
  "searchUi.noResultsBody": "다른 검색어나 지식 종류로 검색해 보세요. 접근할 수 있는 정보만 표시됩니다.",
  "searchUi.startTitle": "어떤 지식이 필요한가요?",
  "searchUi.startBody": "결정, 규칙, 문서와 관계를 검색하세요. Memory를 둘러보거나 첫 문서를 추가할 수도 있습니다.",
  "memoryUi.unavailable": "이 Memory가 없거나 접근 권한이 변경되었습니다.",

  "documentUi.contents.title": "문서 내용",
  "documentUi.contents.description": "처리된 원문을 순서대로 읽습니다. 원본 파일 다운로드가 아닌 추출된 본문입니다.",
  "documentUi.contents.failed": "문서 내용을 읽지 못했습니다. 새로고침하여 다시 시도하세요.",
  "documentUi.contents.loading": "문서 내용 불러오는 중",
  "documentUi.contents.empty": "표시할 처리된 본문이 없습니다.",
  "documentUi.contents.count": "본문 {count}개 표시 중",
  "documentUi.contents.body": "처리된 문서 본문",
  "documentUi.contents.part": "본문 {number}",
  "documentUi.contents.more": "본문 더 보기",
  "result.scope.organization": "조직",
  "result.scope.team": "팀",
  "result.scope.user": "개인",
  "result.scope.unknown": "범위 확인 필요",

};
